...
html"""
<p style="font-size:20px;">The B4lanced: Anthony Dee, Jisoo Kim, Takuma Osaka, Zachary Kwon </br>
...
3. Use an E. coli cell-free system, modified to include the required chemistry to produce the target molecules. Varnerlab will provide specially designed well-mixed reactors ($100 per reactor) with a liquid volume of 100 𝞵L in which the cell-free components are stuck to the reactor. The reactors have two input streams and a single outlet stream. The optimum operating temperature for the cell-free reactors is T = 25oC. Assume all reactors are isothermal and operating at the optimum temperature.
4. Separate unreacted starting materials from final products using specially fabricated universal separator units developed by Varnerlab ($20 each). While these units can magically separate each of the products from each of the sugars (or other required starting materials), they are only 75% efficient (by mass). The separator units are adiabatic and require no power. 
5. Stream splitters or mixing units are available free of charge. No universal cooling or heating jackets are required since everything is isothermal.
6. The reactor units are fed by syringe pumps. Two-channel syringe pumps are available with programmable flow rates between 0.01 to 300 mL/min for $1000 per pump. Assume the pumps have negligible power consumption and can be fed by infinite volume syringes.
7. Analyze the financial performance of the process to determine economic feasibility compared to an alternative investment yielding a minimum of 1% per year and a maximum of 10% per year (constant) using a Net Present Value (NPV) approach.
8. All streams can be assumed to be ideal and at atmospheric pressure. State all other assumptions. 
"""
...
3. ATP + alpha-D-Glucose → ADP + alpha-D-Glucose 6-phosphate
4. alpha-D-Glucose 6-phosphate → beta-D-Fructose 6-phosphate
5. ATP + beta-D-Fructose 6-phosphate → ADP + beta-D-Fructose 1,6-bisphosphate
6. beta-D-Fructose 1,6-bisphosphate → Glycerone phosphate + D-Glyceraldehyde 3-phosphate
7. D-Glyceraldehyde 3-phosphate → Glycerone phosphate
8. Glycerone phosphate → Methylglyoxal + Orthophosphate
...
| ATP | 10.0 | 0.0 | 7.534 | -2.466 |
| NAD+| 0.0  | 0.0 | 4.932 | +4.932 |
| NADH| 10.0 | 0.0 | 5.068 | -4.932 |
| ADP | 0.0  | 0.0 | 2.466 | +2.466 |
| Orthophosphate | 0.0 | 0.0 | 2.466 | +2.466 |
| H+  | 10.0 | 0.0 | 5.068 | -4.932 |
| Sucrose | 10.0 | 0.0 | 8.767 | -1.233 |
| alpha-D-Glucose | 0.0  | 0.0 | 1.233 | +1.233 | 
...
| ATP | 2.466 | 0.0 | 0.0 | -2.466 |
| NAD+| 0.0  | 0.0 | 4.932 | +4.932 |
| NADH| 4.932 | 0.0 | 0.0 | -4.932 |
| ADP | 0.0  | 0.0 | 2.466 | +2.466 |
| Orthophosphate | 0.0 | 0.0 | 2.466 | +2.466 |
| H+  | 4.932 | 0.0 | 0.0 | -4.932 |
| Sucrose | 1.233 | 0.0 | 0.0 | -1.233 |
| alpha-D-Glucose | 0.0  | 0.0 | 1.233 | +1.233 | 
| KOH | 0.0  | 0.0 | 4.932 | +4.932 |
...
3. 2-phospho-D-Glycerate → Phosphoenolpyruvate + H2O
4. ADP + Phosphoenolpyruvate → ATP + Pyruvate
...
| ATP | 0.0 | 0.0 | 0.0 | 0.0 |
| NAD+| 0.0  | 0.0 | 0.0 | 0.0 |
| NADH| 0.0 | 0.0 | 0.0 | 0.0 |
| ADP | 0.0  | 0.0 | 0.0 | 0.0 |
| Orthophosphate | 0.0 | 0.0 | 0.0 | 0.0 |
| **CO2** | 0.0 | 0.0 | 2.466 | +2.466 |
| **Acetate** | 0.0 | 0.0 | 2.466 | +2.466 |
| H+  | 0.0 | 0.0 | 0.0 | 0.0 |
| Sucrose | 1.233 | 0.0 | 0.0 | -1.233 |
| alpha-D-Glucose | 0.0  | 0.0 | 0.0 | 0.0 | 
| KOH | 0.0  | 0.0 | 4.932 | +4.932 |
| KNO3| 4.932 | 0.0 | 0.0 | -4.932 |
| PDGN| 0.0  | 0.0 | 2.466 | +2.466 |
...
begin
	...
	mol_flow_values_feed_1 = [	
		...
		# Still too expensive, because of the high ATP and NADH costs
		# 1.233 ; # sucrose mmol/hr
		# 1.233 ; # h2o mmol/hr
		# 2.466 ; # atp mmol/hr
		# 4.932 ; # nadh mmol/hr
		...
		# These values led to the presence of excess reactants in the output stream (not optimum!)
		# 10.0  ; # sucrose mmol/hr
		# 10.0  ; # h2o mmol/hr
		# 10.0  ; # atp mmol/hr
		# 10.0  ; # nadh mmol/hr
		# 10.0  ; # h mmol/hr
		# 10.0  ; # potassium nitrate mmol/hr
	]
...
md"""
### Conclusions
The client’s idea of creating PGDN with sucrose is financially sustainable depending on how much time the client is willing to spend on this project. Even if the depreciation rate is 1% or 10%, as long as the client is willing to run the reactor for more than approximately two years (NPV becomes positive starting in the second year), the idea is financially sustainable. However, if the client is just looking to get PGDN quickly, it is more financially efficient to buy the chemicals directly from SigmaAldrich. In conclusion, the project is financially stable depending on the client's situation. 
"""

# ╔═╡ 2f2713eb-a958-4d1a-a1cc-2723ea13c38c
md"""
### References
Throughout this project, we referred to various secondary sources. Below is a list of our Reference Sources:
- Model_v2.vff file
- Example-Chip-FluxBalanceAnalysis-Project.jl
- Example-Chip-FluxBalanceAnalysis-PDO-Parallel.jl
- Example-Chip-FluxBalanceAnalysis-Series.jl
- “Millipore Sigma.” MilliporeSigma | United States, https://www.sigmaaldrich.com/US/en. 
- “Propylene Glycol Dinitrate.” National Center for Biotechnology Information. PubChem Compound Database, U.S. National Library of Medicine, https://pubchem.ncbi.nlm.nih.gov/compound/Propylene-glycol-dinitrate. 
- “Sucrose.” Encyclopædia Britannica, Encyclopædia Britannica, Inc., https://www.britannica.com/science/sucrose.
"""
